...
# ToDo: probably no need to pass the device to the oracles (zeros_like should use same device) check this
...
class SparseBall:
    ...
    """
    def __init__(self, diameter=None, radius=None, k=1):
        self.k = k
...
class InitializerSparseBall:
    # Polytopes with vertices v \in {0, +/- r}^n such that exactly k entries are nonzero
    def __init__(self, width=3, k=1):
        self.width = width
        self.k = k
...
class InitializerLpBall:
    def __init__(self, width=3, ord=2):
...
class LpBall:
    ...
    """
    def __init__(self, diameter=None, radius=None, ord=2):
        self.p = float('inf') if ord=='inf' else ord
